...
namespace BehaviorDesigner.Runtime.Tasks.Movement
{
    ...
    public class WithinDistance : Conditional
    {
        ...
        // true if we obtained the objects through the objectTag
        private bool dynamicObjects;
        ...
        public override void OnAwake()
        {
            ...
            dynamicObjects = objects.Value != null && objects.Value.Count == 0;
        }
        ...
        public override TaskStatus OnUpdate()
        {
            ...
            for (int i = 0; i < objects.Value.Count; ++i) {
                direction = objects.Value[i].position - transform.position;
                ...
                if (Vector3.SqrMagnitude(direction) < sqrMagnitude) {
                    ...
                    if (lineOfSight.Value) {
                        ...
                        is2D = usePhysics2D;
#endif
                        if (MovementUtility.LineOfSight(transform, objects.Value[i], is2D)) {
        ...
        {
            // set the objects array to null if dynamic objects is true so the objects will be found again the next time the task starts
            if (dynamicObjects) {
                objects.Value = null;
            }
        }
