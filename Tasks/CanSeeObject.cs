        ...
        public override TaskStatus OnUpdate()
        {
            ...
            if (usePhysics2D) {
                ...
                if (targetObject.Value == null) {
                    objectInSight.Value = MovementUtility.WithinSight2D(transform, fieldOfViewAngle.Value, viewDistance.Value, objectLayerMask);
                } else { // If the target is not null then determine if that object is within sight
                    objectInSight.Value = MovementUtility.WithinSight2D(transform, fieldOfViewAngle.Value, viewDistance.Value, targetObject.Value);
                }
            } else {
#endif
                // If the target object is null then determine if there are any objects within sight based on the layer mask
                if (targetObject.Value == null) {
                    objectInSight.Value = MovementUtility.WithinSight(transform, fieldOfViewAngle.Value, viewDistance.Value, objectLayerMask);
                } else { // If the target is not null then determine if that object is within sight
                    objectInSight.Value = MovementUtility.WithinSight(transform, fieldOfViewAngle.Value, viewDistance.Value, targetObject.Value);
                }
#if !(UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2)
            }
#endif
