...
#if !(UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2)
                if (usePhysics2D) {
                    objectHeard.Value = MovementUtility.WithinHearingRange2D(transform, linearAudibilityThreshold.Value, hearingRadius.Value, objectLayerMask);
                } else {
#endif
                    objectHeard.Value = MovementUtility.WithinHearingRange(transform, linearAudibilityThreshold.Value, hearingRadius.Value, objectLayerMask);
#if !(UNITY_4_0 || UNITY_4_0_1 || UNITY_4_1 || UNITY_4_2)
                }
#endif
            } else { // If the target is not null then determine if that object is within sight
                objectHeard.Value = MovementUtility.WithinHearingRange(transform, linearAudibilityThreshold.Value, targetObject.Value);
            }
